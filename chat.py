...
async def chatbot_flow(db_object, chat_id, message, message_type, acknowledgements):
    ...
    if not await db_object.check_failure_exist(chat_id=chat_id):
        await db_object.insert_user_prompt(
            chat_id=chat_id,
            conversation_chunk_id="FAILURE" + str(chat_id),
            scheme_name=None,
    ...
    if message_type == "text":
        ...
        (
            message,
            user_regional_lang_text,
            vernacular_to_english_translation_success,
            vernacular_to_english_translation_api_name,
        ) = process_incoming_text(message, translate)
    else:
        ...
        if "audio/ogg" in audio_response.headers.get("Content-Type"):
            ...
            (
                ...
                audio_file, language_preference, translate=translate
            )
            user_audio_file_link = await upload_file_and_get_public_url(
                filename=inbound_audio_file_name,
    ...
    if (stt_success and vernacular_to_english_translation_success) or (
        ...
        if message_type != "text":
            temp_response = (
                acknowledgements[language_preference] + user_regional_lang_text
            )
        ...
        if bot_preference == "scheme_v1" or bot_preference is None:
            (
                ...
                next_prompt,
                llm_output,
            ) = await scheme_v1(db_object, message, chat_id)
        ...
        if (
            davinci_response
            == "Because Server is overloaded, I am unable to answer you at the moment. Please retry."
        ):
            davinci_success = False
        else:
        ...
        if (
            davinci_response
            == "Sorry, I could not understand that information. Please answer in different wording."
        ):
            bot_success = False
        else:
            bot_success = True
        ...
        (
            _,
            regional_lang_text,
            english_to_vernacular_translation_success,
            english_to_vernacular_translation_api_name,
        ) = process_outgoing_text(message=davinci_response, translate=translate)
        ...
        current_scheme_conversation_summary = (
            current_scheme_conversation_summary.replace("'", "")
        )

        await db_object.update_user_prompt(
        ...
        if message_type != "text":
            # perform TTS
            audio, duration_seconds, tts_service_name = process_outgoing_voice(
                message=regional_lang_text, translate=translate
            )

            if audio:
                tts_success = True
                # send the audio response to user
                outbound_audio_file_name = (
                    str(chat_id)
                    + "-Outbound-"
                    + str(datetime.datetime.now()).replace(" ", "-")
                    + ".mp3"
                ...
                bot_audio_file_link = await upload_file_and_get_public_url(
                    filename=outbound_audio_file_name,
                    phone_number=chat_id,
        ...
        await db_object.insert_conversation(
            chat_id=chat_id,
            user_audio_file_link=user_audio_file_link,
            ...
            next_prompt_name=next_prompt_name,
            next_prompt=next_prompt,
            llm_output=llm_output,
        )
        # insert service log
        await db_object.insert_service_logs(
            chat_id=chat_id,
            conversation_chunk_id=new_conversation_chunk_id,
            created_at=date,
            message_type=message_type,
            bot_preference=bot_preference,
            davinci_success=davinci_success,
            ...
            stt_api_success=stt_success,
            stt_api_name=stt_api_name,
