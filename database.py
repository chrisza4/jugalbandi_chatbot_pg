...
class PostgresDatabase:
    ...
    async def update_api_quota(self, api_key):
        async with self.engine.acquire() as connection:
            ...
            await connection.execute(
                "Update jugalbandi_tokens set used_quota = $1 WHERE api_key = $2",
                existing_user["used_quota"] + 1,
                api_key,
            )
    ...
    async def check_api_key(self, api_key):
        ...
        if (
            existing_user
            and existing_user["available_quota"] > existing_user["used_quota"]
        ):
            return True
        else:
            return False
    ...
    async def insert_service_logs(
        ...
        davinci_success,
        bot_success,
        vernacular_to_english_translation_api_success,
    ...
    async def get_bot_preference(self, chat_id):
        async with self.engine.acquire() as connection:
            result = await connection.fetchrow(
                "SELECT bot_preference FROM jugalbandi_users WHERE chat_id = $1",
                chat_id,
            )
    ...
    async def get_language_preference(self, chat_id):
        async with self.engine.acquire() as connection:
            result = await connection.fetchrow(
                "SELECT language_preference FROM jugalbandi_users WHERE chat_id = $1",
                chat_id,
            )
    ...
    async def insert_conversation(
        ...
        user_message,
        bot_response,
        user_message_translated,
        bot_response_translated,
        date,
        language_preference,
        current_prompt,
        next_prompt_name,
    ...
    async def update_user_prompt_with_new_scheme(
        self, chunk_id, scheme_name, prompt_type
    ):
        async with self.engine.acquire() as conn:
            await conn.execute(
                """
    ...
    async def get_specific_scheme_details(self, scheme_name) -> dict:
        ...
        async with self.engine.acquire() as connection:
            result = await connection.fetchrow(
                f"SELECT * FROM schemes_details where scheme_name='{scheme_name}'"
            )
        if result is not None:
            specific_scheme_details["scheme_name"] = result["scheme_name"]
            specific_scheme_details["summary"] = result["summary"]
            specific_scheme_details["eligibility_criteria"] = result[
    ...
    async def get_specific_scheme_details_myscheme(self, scheme_name) -> dict:
        ...
        async with self.engine.acquire() as connection:
            result = await connection.fetchrow(
                f"SELECT * FROM myschemes_info where scheme_name='{scheme_name}'"
            )
        if result is not None:
            specific_scheme_details["scheme_name"] = result["scheme_name"]
            ...
            specific_scheme_details["benefits"] = result["benefits"]
            specific_scheme_details["application_process"] = result[
                "application_process"
            ]
            specific_scheme_details["documents_required"] = result["documents_required"]
    ...
    async def get_prompts(self, chat_id):
        ...
        if result is None:
            ...
            scheme_chatbot_prompt = json.load(open("data/prompts.json"))[
                "scheme_chatbot_prompt"
            ]
            current_prompt, current_scheme_summary = await load_specific_scheme_prompt(
                current_scheme_name, scheme_chatbot_prompt, self.engine
            )
    ...
    async def get_prompts_v1(self, chat_id):
        ...
        if result is None or "FAILURE" in result["conversation_chunk_id"]:
            ...
            if current_prompt_type == "specific_scheme_conversation":
                ...
                current_prompt, scheme_summary = load_specific_scheme_prompt(
                    current_scheme_name, scheme_chatbot_prompt
                )
                ...
                current_prompt = (
                    all_prompts["scheme_name_disambiguation_filtered_schemes"]
                    + "\n"
                    + "\n".join(current_scheme_name.split("||"))
                )
            else:
                current_prompt = ""
