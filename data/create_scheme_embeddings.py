...
def create_scheme_embeddings(scheme_data):
    max_characters = (
        8000 * 4
    )  #### max token limit for OpenAI is 8191 and roughly 4 characters per token
    scheme_summaries = []
    for scheme in scheme_data:
        ...
        if len(scheme_summary) > max_characters:
            ...
            scheme_summary_prompt = (
                "Summarize the text below in less than 6000 words: \n\n"
                + scheme["details"]
            )
            scheme_summary = (
        ...
        scheme_summaries.append(
            Document(
                page_content=scheme_summary, metadata={"source": scheme["scheme_name"]}
            )
        )
...
def create_scheme_summary(scheme_data):
    max_characters = (
        4000 * 4
    )  #### max token limit for OpenAI is 4096 and roughly 4 characters per token
    for scheme in tqdm(scheme_data):
        if (
            scheme.get("summary") is None
            or scheme.get("summary")
            ...
            if len(scheme_info) > max_characters:
                scheme_info = scheme_info[
                    :max_characters
                ]  ## truncate long scheme informations

            scheme_summary_prompt = (
                "Create summary of following text in 2 lines. Summary should cover target audience of scheme and benefits. it is important to be factually correct and only use the information given below.\n\n"
                + scheme_info
            )
            try:
