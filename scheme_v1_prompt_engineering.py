...
def load_specific_scheme_prompt(
    interested_scheme_name, scheme_chatbot_prompt
) -> (str, str):
    """
    ...
    if interested_scheme_details:
        ...
        if interested_scheme_details["eligibility_criteria"]:
            ...
            prompt = (
                prompt
                + "\n\nScheme Name: "
                + interested_scheme_name
                + "\n\nStart of scheme "
                'details\n\nEligibility_Criteria:\n"""\nApplicant may be eligible for '
                + interested_scheme_name
                + " if he/she meet the following criteria:\n"
                + str(eligibility)
                + '"""'
            )
...
def get_best_matching_scheme(
    question, user_need_scheme_category, do_scheme_category_filtering, k=10
) -> list[str]:
    exact_match_threshold = 0.3
    score_threshold = 0.5
    best_matching_schemes = []

    eligible_schemes_chunks = search_index.similarity_search_with_score(
        query=question, k=k
    )
    if eligible_schemes_chunks:
        ...
        exact_matches = [
            i.metadata["source"]
            for (i, score) in eligible_schemes_chunks
        ...
        if do_scheme_category_filtering:
            for eligible_scheme in eligible_schemes:
                ...
                if (
                    user_need_scheme_category
                    in all_schemes_information_dict.get(eligible_scheme_name)[
                        "schemeCategory"
                    ]
                ):
...
def create_user_response_for_best_matching_schemes(best_matching_schemes):
    ...
    if len(best_matching_schemes) > 0:
        ...
        for i, scheme in enumerate(best_matching_schemes):
            ...
            if (
                all_schemes_information_dict.get(scheme) is not None
                and len(best_matching_schemes) <= max_schemes_to_show
            ):
                user_response = (
                    user_response
                    + ": "
...
def create_user_response_for_too_many_matching_schemes(best_matching_schemes):
    if len(best_matching_schemes) > 0:
        ...
        user_response = (
            "I found "
            + str(len(best_matching_schemes))
            + " schemes that could be useful, but I need more information to find more relevant ones. Alternatively, please let me know which one of these schemes you would like to know more about."
        )
        for i, scheme in enumerate(best_matching_schemes):
            user_response = user_response + "\n" + str(i + 1) + ") " + scheme
    else:
...
def create_prompt_for_filtered_scheme_name_disambiguation(best_matching_schemes):
    ...
    scheme_name_disambiguation_filtered_schemes_prompt = (
        scheme_name_disambiguation_filtered_schemes_prompt + concatenated_scheme_names
    )
...
class scheme_chatbot_fsm:
    states = [
        "user_information_extraction",
        "specific_scheme_name_disambiguation",
        "specific_scheme_conversation",
    ]
    ...
    def __init__(
        ...
        self.machine = Machine(
            model=self, states=scheme_chatbot_fsm.states, initial=wake_up_state
        )
        ...
        self.machine.add_transition(
            trigger="user_selected_scheme_from_options",
        ...
        self.machine.add_transition(
            trigger="continue_scheme_conversation",
            source="specific_scheme_conversation",
            dest="specific_scheme_conversation",
    ...
    def check_specific_need_or_specific_scheme(self) -> bool:
        ...
        try:
            ...
            user_information_str = re.sub(
                r"(^.*)({\"Extracted_Info.*)", r"\2", self.llm_output, flags=re.DOTALL
            )
    ...
    def check_if_single_scheme_selected_from_options(self) -> bool:
        ...
        try:
            ...
            user_information_str = re.sub(
                r"(^.*)({\'user_selected_filtered_scheme.*)",
                r"\2",
                output,
                flags=re.DOTALL,
            )
            ...
            if (
                user_information_dict["user_selected_filtered_scheme"]
                == "MULTIPLE_MATCHES"
            ):
                return False

            else:
                self.next_scheme_name = user_information_dict[
                    "user_selected_filtered_scheme"
                ]
                return True
    ...
    def check_if_ambiguous_scheme_selected_from_options(self) -> bool:
        ...
        try:
            ...
            if (
                ...
                self.scheme_name = user_information_dict[
                    "user_selected_filtered_scheme"
                ]
                return False
    ...
    def perform_user_need_extraction_after_scheme_change(self):
        ...
        self.llm_output_logging = (
            self.llm_output + prompts_seperator + copy.deepcopy(output)
        )
        self.current_scheme_conversation_summary = (
            ""  ## clear memory as we are restarting the conversation
        )
    ...
    def search_scheme_for_user_need(self):
        ...
        score_threshold = 0.5
        if (
            self.user_information_dict["Extracted_Info"]["Specific_Scheme_Information"]
            == "Yes"
        ):
        ...
        faiss_search_results = search_index.similarity_search_with_score(
            query=query, k=k
        )
        faiss_filtered_schemes = [
            i.metadata["source"]
            for (i, score) in faiss_search_results
            if score <= score_threshold
        ]

        faiss_filtered_schemes_with_summary = self.get_scheme_summaries(
            faiss_filtered_schemes
        )
        if k == 1:
            ...
            if os.environ.get("SCHEME_SEARCH_FILTERING") == "cot_filtering":
                ...
                if self.search_model == "chatgpt":
                    chatgpt_system_prompt = (
                        prompts["scheme_filtering_chatgpt_prompt"]
                        + faiss_filtered_schemes_with_summary
                    )
                    ...
                    scheme_filtering_llm_response = call_chatgpt_api(
                        messages, max_tokens=1024
                    )
                else:

                    prompt_and_schemes = (
                        prompts["scheme_filtering_prompt"]
                        + faiss_filtered_schemes_with_summary
                        + '"""\n'
                    )
                    prompt = (
                    ...
                    scheme_filtering_llm_response = call_openAI_api(
                ...
                self.llm_output_logging = (
                    self.llm_output
                    + prompts_seperator
                    + copy.deepcopy(scheme_filtering_llm_response)
                )
                self.parse_scheme_filtering_llm_output(scheme_filtering_llm_response)
            else:
                user_expressed_scheme_category = self.user_information_dict[
                    "Extracted_Info"
                ]["Scheme_Category"]
                self.scheme_search_result = get_best_matching_scheme(
                    self.user_input,
                    user_expressed_scheme_category,
                    do_scheme_category_filtering,
                    k,
                )
    ...
    def update_user_response_and_next_prompt_for_ambiguous_scheme_selection(self):
        ...
        self.next_prompt = create_prompt_for_filtered_scheme_name_disambiguation(
            best_matching_schemes
        )
    ...
    def update_user_response_and_next_prompt_for_single_matching_schemes(self):
        ...
        self.next_prompt, scheme_summary = load_specific_scheme_prompt(
            single_best_scheme, prompts["scheme_chatbot_prompt"]
        )
    ...
    def update_user_response_and_next_prompt_for_specific_scheme_continuation(self):
        if "user_message" in self.llm_output:
            user_message_str = re.sub(
                r"(^.*)({\"user_message.*)", r"\2", self.llm_output, flags=re.DOTALL
            )
            try:
    ...
    def parse_scheme_filtering_llm_output(self, scheme_filtering_llm_response):
        clarifying_questions = re.findall(
            r"{\s*\'user_message\':.*}", scheme_filtering_llm_response, re.DOTALL
        )
        try:
            self.clarifying_question = ast.literal_eval(clarifying_questions[-1])[
                "user_message"
            ]
        except:
        ...
        scheme_search_result_str = re.search(
            r"relevant_schemes_list.*(\[.*\])", scheme_filtering_llm_response
        ).groups()[0]
        try:
...
async def get_scheme_fsm_bot_response(
    current_prompt,
    current_state,
    current_scheme_conversation_summary,
    user_input,
    db_obj,
    current_conversation_chunk_id,
    current_scheme_name,
    user_id,
    bot_preference,
):
    ...
    machine_fsm = scheme_chatbot_fsm(
        wake_up_state=current_state,
    ...
    try:
        ...
        current_scheme_conversation_summary = (
            machine_fsm.current_scheme_conversation_summary
        )
        prompt = (
            machine_fsm.current_prompt
            + conversation_history_prefix
            + current_scheme_conversation_summary.strip()
            + "\n\nUser: "
            + user_input
    ...
    if (
        new_conversation_chunk_id == current_conversation_chunk_id
        and new_conversation_chunk_id != ""
    ):
        await db_obj.update_user_prompt_with_new_scheme(
            new_conversation_chunk_id, scheme_name, next_prompt_type
        )
    else:
        import uuid

        new_conversation_chunk_id = uuid.uuid4().hex
        await db_obj.insert_user_prompt(
            chat_id=user_id,
