...
def process_incoming_voice(audio_bytes_data, language_preference, translate):
    try:
        regional_lang_text, api_name = translate.audioInput2text(
            audio_data=audio_bytes_data
        )
        s2t_success = True
        try:
            message, translation_api_name = translate.indicTrans(
                text=regional_lang_text, source=language_preference, dest="en"
            )
...
def process_outgoing_voice(message, translate):
    try:
        audio_content, is_google = translate.text2speech(
            text=message, language=translate.input_language
        )
...
def process_incoming_text(message, translate):
    ...
    try:
        message, translation_api_name = translate.indicTrans(
            text=message, source=translate.input_language, dest="en"
        )
        success = True
    except:
...
def process_outgoing_text(message, translate):
    try:
        regional_lang_text, translation_api_name = translate.indicTrans(
            text=message, source="en", dest=translate.input_language
        )
        success = True
    except:
        success = False
